...
export function SplitFlapMuteToggle({ className = "" }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={`inline-flex cursor-pointer items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-dim transition-colors duration-200 hover:text-ink ${className}`}
...
interface SplitFlapTextProps {
  ...
  className?: string;
  speed?: number;
  /** CSS font-size for each tile — tune so the full board fits the viewport. */
  fontSize?: string;
}
...
function SplitFlapChar({
  ...
  useEffect(() => {
    ...
    // Space tiles render a fixed spacer and never read this state.
    if (isSpace) return;

    const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    const baseFlips = 8;
    const startDelay = reduceMotion ? 0 : skipEntrance ? tileDelay * 400 : tileDelay * 800;
    let flipIndex = 0;
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-1/2 top-0 flex items-end justify-center overflow-hidden">
        <span
      ...
      <div className="absolute inset-x-0 bottom-0 top-1/2 flex items-start justify-center overflow-hidden">
        <span
      ...
      <motion.div
        ...
        className="absolute inset-x-0 bottom-1/2 top-0 origin-bottom overflow-hidden"
        style={{
          backgroundColor: bgColor,
          transformStyle: "preserve-3d",
